...
const timerProgress =
  document.querySelector<SVGCircleElement>('.timer-progress');
    ...
    ${groups
      ...
      <div class="stretch-group">
        <header>
          ...
          ${
            i > 0
              ? `<button data-move-up="${i}" type="button" title="Move Up">↑</button>`
              : ``
          }
          ${
            i !== groups.length - 1
              ? `<button data-move-down="${i}" type="button" title="Move Down">↓</button>`
              : ``
          }
        ...
        <section>
          ${Object.entries(group)
            .map(([key, value]) => renderFormInputs(key, value as string))
            .join('')}
        </section>
      </div>
        `
...
//       .map((group) =>
//         Array.from({ length: group.repetitions }, () => {
//           const stretch = { ...group };
...
async function run(stretches: StretchGroup[]) {
  ...
  for (const stretch of stretches) {
    ...
    for (const repetition of Array.from({ length: stretch.repetitions })) {
      ...
      timerProgress?.style.setProperty(
        '--transitionLength',
        `${stretch.repetitionLength}s`
      );
...
startButton?.addEventListener('click', () =>
  run(schedule).catch(console.error)
);
...
// Delete / Moove
editor?.addEventListener('click', (event: Event) => {
...
editor?.addEventListener('submit', (event: Event) => {
  event.preventDefault();
  if (!(event.target instanceof HTMLFormElement)) return;
  const form = event.target;
  const data = new FormData(form);
  const newStretchGroup = Object.fromEntries(
    data.entries()
  ) as unknown as StretchGroup;
