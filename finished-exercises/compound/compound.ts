...
const compoundForm = document.querySelector<CompoundFormElement>('form[name="compound"]');
...
function getYearlyResults(): YearlyResult[] {
  ...
  return yearsArray.map(year => {
    const currentValue = fv(values.return / values.frequency, values.frequency * year, values.contribution * -1, values.initial * -1, PaymentDueTime.End);
    const contribution = ((values.contribution * values.frequency) * year) + values.initial;
...
function generateSingleYearHTML(result: YearlyResult, prevResult: YearlyResult, finalResult: YearlyResult) {
  ...
  return html`
    <div class="year">
      ...
      <div class="result" style="width: ${(result.currentValue / finalResult.currentValue) * 100}%; grid-template-columns: ${cWidth}% ${pWidth}%">
        <span class="contribution">
          ${formatMoney(result.contribution)}
        </span>
        <span class="profit">
        ${formatMoney(result.profit)}
        </span>
        <p class="yearlyBump green">+${formatMoney(prevResult ? result.currentValue - prevResult.currentValue : result.contribution + result.profit)}</p>
      </div>
    </div>
  `;
}
...
function generateResultsHTML(results: YearlyResult[]): string {
  ...
  return html`
    <div>
      <h2>You'll have ${formatMoney(finalResult.currentValue)} in ${results.length} years!</h2>
      <div class="years">
      ${results.map((result, i) => generateSingleYearHTML(result, results[i-1], finalResult)).join('')}
      </div>
    </div>
    `;
  }
